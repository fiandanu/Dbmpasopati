...
{
    private function calculatePdfStatus($vtren)
    {
        $uploadedFolders = 0;
        $totalFolders = 10;
    ...
    private function applyFilters($query, Request $request)
    {
        ...
        if ($request->has('table_search') && !empty($request->table_search)) {
            ...
            $query->where(function ($q) use ($searchTerm) {
                $q->where('tutor_ponpes_vtren', 'LIKE', '%' . $searchTerm . '%')
        ...
        if ($request->has('search_judul') && !empty($request->search_judul)) {
            $query->where('tutor_ponpes_vtren', 'LIKE', '%' . $request->search_judul . '%');
        }
    ...
    public function tutorial_ponpes_vtren(Request $request)
    {
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'tutor_ponpes_vtren' => 'required|unique:tutor_ponpes_vtren,tutor_ponpes_vtren',
        ], [
            'tutor_ponpes_vtren.required' => 'Nama tutor wajib diisi.',
            'tutor_ponpes_vtren.unique' => 'Nama tutor sudah ada, silakan gunakan nama lain.',
        ]);
        ...
        return redirect()->route('tutor_ponpes_vtren.ListDataSpp')->with('success', 'Data berhasil disimpan');
    }
    ...
    public function ListDataSpp(Request $request)
    {
        ...
        return view('tutorial.ponpes.vtren', compact('data'))->with('title', 'Tutorial Ponpes vtren');
    }
    ...
    public function exportListCsv(Request $request): StreamedResponse
    {
        ...
        $filename = 'list_tutorial_vtren_' . Carbon::now()->format('Y-m-d_H-i-s') . '.csv';
        ...
        $rows = [['No', 'Tutorial Vtren', 'Tanggal Dibuat', 'Status Upload PDF']];
        $no = 1;
        foreach ($data as $d) {
            $status = $this->calculatePdfStatus($d);
            $rows[] = [
                $no++,
                $d->tutor_ponpes_vtren,
    ...
    public function exportListPdf(Request $request)
    {
        ...
        $pdfData = [
            'title' => 'List Tutorial Vtren',
            'data' => $dataArray,
            'generated_at' => Carbon::now()->format('d M Y H:i:s')
        ];

        $pdf = Pdf::loadView('export.public.tutorial.ponpes.indexVtren', $pdfData);
        $filename = 'list_tutorial_vtren_' . Carbon::now()->translatedFormat('d_M_Y') . '.pdf';
    ...
    public function DatabasePageDestroy($id)
    {
        try {
            ...
            $dataupt->delete();
            return redirect()->route('tutor_ponpes_vtren.ListDataSpp')->with('success', 'Data berhasil dihapus');
        } catch (\Exception $e) {
            Log::error('Error deleting Vtren: ' . $e->getMessage());
            return redirect()->route('tutor_ponpes_vtren.ListDataSpp')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
    ...
    public function uploadFilePDF(Request $request, $id, $folder)
    {
        try {
            ...
            $directory = 'tutorial/ponpes/vtren/folder_' . $folder;
            ...
            Log::info("PDF uploaded successfully for Vtren ID: {$id}, Folder: {$folder}, Path: {$path}");

            return redirect()->back()->with('success', 'PDF berhasil di-upload ke folder ' . $folder . '!');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->back()->withErrors($e->errors())->withInput();
        } catch (\Exception $e) {
            Log::error('Error uploading PDF Vtren: ' . $e->getMessage());
    ...
    public function deleteFilePDF($id, $folder)
    {
        try {
            ...
            Log::error('Error deleting PDF Vtren: ' . $e->getMessage());
