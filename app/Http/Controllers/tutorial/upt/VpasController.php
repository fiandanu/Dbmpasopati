...
class VpasController extends Controller
{
    ...
    public function DatabasePageDestroy($id)
    {
        try {
            ...
            $dataupt->delete();
            return redirect()->route('tutor_vpas.ListDataSpp')->with('success', 'Data berhasil dihapus');
        } catch (\Exception $e) {
            return redirect()->route('tutor_vpas.ListDataSpp')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
